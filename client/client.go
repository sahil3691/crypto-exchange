...
import (
	...
	"github.com/sahildhargave/crypto/server"
)
...
func (c *Client) GetOrders(userID int64) (*server.GetOrdersResponse, error){
	   ...
	   if err := json.NewDecoder(resp.Body).Decode(&orders);
	   err != nil {
		return nil, err 
	   }
	   return &orders, nil 
...
func (c *Client) PlaceMarketOrder(p *PlaceOrderParams)  (*server.PlaceOrderResponse, error){
	...
	if err := json.NewDecoder(resp.Body).Decode(placeOrderResponse);
	err != nil {
		return nil, err 
	}
...
func (c *Client) GetBestAsk() (*server.Order, error) {
	...
	order := &server.Order{}
	if err := json.NewDecoder(resp.Body).Decode(order);
	err != nil {
		return nil,err 
	}
	return order, err 
